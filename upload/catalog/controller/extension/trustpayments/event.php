...
class ControllerExtensionTrustPaymentsEvent extends TrustPayments\Controller\AbstractEvent {
	...
	public function includeCronScript(){
		...
		$security_token = \TrustPayments\Entity\Cron::getCurrentSecurityTokenForPendingCron($this->registry);
		if ($security_token) {
			$cronUrl = $this->createUrl('extension/trustpayments/cron', array(
	...
	public function canSaveOrder(){
		...
		if (!empty($new_order)) {
			\TrustPaymentsHelper::instance($this->registry)->log($this->language->get('error_order_edit') . " ($order_id)", \TrustPaymentsHelper::LOG_ERROR);
